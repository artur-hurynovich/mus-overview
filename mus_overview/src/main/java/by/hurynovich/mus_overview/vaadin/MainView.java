...
public class MainView extends VerticalLayout {
    ...
    private Grid<OverviewDTO> overviewsGrid;
    private Grid<OverviewDTO> searchGrid;
    ...
    public MainView(final GroupService groupService, final OverviewService overviewService) {
        ...
        Tab overviewsTab = buildOverviewsTab();
        Tab searchTab = buildSearchTab();
        VerticalLayout overviewsLayout = buildOverviewsLayout();
        VerticalLayout searchLayout = buildSearchLayout();
        Tabs tabs = new Tabs(overviewsTab, searchTab);
    ...
    private VerticalLayout buildOverviewsLayout() {
        VerticalLayout overviewsLayout = new VerticalLayout();
        HorizontalLayout showParametersLayout = new HorizontalLayout();
        showParametersLayout.setDefaultVerticalComponentAlignment(Alignment.BASELINE);
        ComboBox<GroupDTO> groupsComboBox = buildGroupsComboBox();
        ComboBox<SubgroupDTO> subgroupsComboBox = buildSubgroupsComboBox();
        Button showOverviewsButton = buildShowOverviewsButton();
    ...
    private VerticalLayout buildSearchLayout() {
        ...
        TextField searchTextField = buildSearchTextField();
    ...
    private TextField buildSearchTextField() {
        ...
        tagsTextField.addValueChangeListener(e -> {
            String searchField = tagsTextField.getValue();
            if (!searchField.isEmpty()) {
                String[] tags = searchField.split(" ");
                List<OverviewDTO> overviewsByTags = overviewService.getAllOverviewsByTags(tags);
    ...
    private Button buildShowOverviewsButton() {
        Button showOverviewsButton = new Button("Show Overviews");
    ...
    private Grid<OverviewDTO> buildShowGrid() {
        Grid<OverviewDTO> showGrid = new Grid<>();
